...
import hobbit from '../images/hobbit.mp4'
  ...
  const fetchData = async () => {
    axios.post('https://id.twitch.tv/oauth2/token?client_id=5pc7pjbfvo2tm964nyjv5795uk328s&client_secret=vqiqhqa1w944choph1zo5j9la0zh50&grant_type=client_credentials')
      .then(async response => {
        ...
        api.get('https://api.twitch.tv/helix/streams?user_login=zeylda')
          .then(response => {
        ...
        api.get('https://api.twitch.tv/helix/users?login=zeylda')
          .then(response => {
            setTotalView(response.data.data[0].view_count)
          })
        api.get('https://api.twitch.tv/helix/users/follows?to_id=421475350')
          .then(response => {
            setTotalFollower(response.data.total)
          })
        api.get('https://api.twitch.tv/helix/schedule?broadcaster_id=421475350')
          .then(response => {
    ...
    <div className='font-wbz'>
      <main className="mx-auto">
        <div className="text-center relative w-screen h-screen">
          <video className='w-full h-full object-cover' src={hobbit} autoPlay loop muted />
            ...
            <div className='h-full flex flex-col justify-center items-center'>
              <div className='bg-slate-700 bg-opacity-80 rounded-3xl py-4 lg:py-8 px-2 mx-2 my-4'>
                <h1 className="text-4xl lg:text-5xl font-bold tracking-tight text-gray-900 ">
                  <span className="block xl:inline text-white">Hellooooow ! </span>
                  <span className="block text-white xl:inline">Bienvenue dans la taverne !</span>
                </h1>
                <p className="mx-auto mt-3 max-w-md text-lg sm:text-xl text-white md:mt-5 md:max-w-4xl lg:text-2xl">
                ...
                <div className="mx-auto mt-5 max-w-md sm:flex sm:justify-center md:mt-8">
                  <div className="rounded-md shadow">
                    <Link href="https://www.twitch.tv/zeylda" className="justify-center">
                      <a target="_blank" className='heartbeat'>
                        <button type="button" className="inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-6 py-3 text-xl lg:text-2xl font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
        ...
        <div className="bg-white">
          <div className="mx-auto max-w-7xl py-12 px-4 sm:px-6 lg:py-16 lg:px-8">
            <p className="text-center text-3xl font-semibold text-gray-600">Mes replays</p>
            <div className="mt-6 grid grid-cols-2 gap-0.5 md:grid-cols-3 lg:mt-8">
              <div className="col-span-1 flex justify-center bg-gray-50 py-8 px-8">
                <Image className="max-h-78" sizes="" src={image6} alt="Workcation" />
              </div>
              <div className="col-span-1 flex justify-center bg-gray-50 py-8 px-8">
                <Image className="max-h-78" src={image6} alt="Mirage" />
              </div>
              <div className="col-span-1 flex justify-center bg-gray-50 py-8 px-8">
                <Image className="max-h-78" src={image6} alt="Tuple" />
              </div>
              <div className="col-span-1 flex justify-center bg-gray-50 py-8 px-8">
                <Image className="max-h-78" src={image6} alt="Laravel" />
              </div>
              <div className="col-span-1 flex justify-center bg-gray-50 py-8 px-8">
                <Image className="max-h-78" src={image6} alt="StaticKit" />
              </div>
              <div className="col-span-1 flex justify-center bg-gray-50 py-8 px-8">
                <Image className="max-h-78" src={image6} alt="Statamic" />
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="mx-auto max-w-7xl py-12 px-4 sm:px-6 lg:px-8 lg:py-24">
            <div className="grid grid-cols-1 gap-12 lg:grid-cols-3 lg:gap-8">
              <div className="space-y-5 sm:space-y-4">
                <h2 className="text-3xl text-center font-bold tracking-tight sm:text-4xl text-amber-400 bg-opacity-90 bg-amber-900 rounded-3xl p-2">Les gardiens de la taverne</h2>
              </div>
              <div className="lg:col-span-2">
                <ul role="list" className="space-y-12 sm:grid sm:grid-cols-2 sm:gap-12 sm:space-y-0 lg:gap-x-8">
                  {moderators.map((moderator) => (
                    <li>
                      <div key={moderator.name} className="flex items-center space-x-4 lg:space-x-6 bg-amber-900 bg-opacity-90 rounded-3xl p-2">
